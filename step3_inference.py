...
class Inference(object):
    def __init__(self, path = FinetunePath, mode='s'):
        self.char_count = 0
        self.top1_acc = 0
        self.top5_acc = 0
        self.sen_count = 0
    ...
    def get_topk(self, text, k=5):
        ...
        with torch.no_grad():
            ...
            output_topk_prob = torch.topk(output_tensor, k).values.squeeze(0).tolist()
            output_topk_indice = torch.topk(output_tensor, k).indices.squeeze(0).tolist()
